...
public interface UserMapper {
    String fieldListStr = "u_id AS id, u_skill AS skill, u_name AS name, u_type AS type, u_power AS power, create_time AS createTime, update_time AS updateTime";

    @Insert("INSERT INTO user(u_id, u_skill, u_name, u_power, u_type) " +
            "VALUES(#{id}, #{skill}, #{name}, #{power}, #{type})")
    int insertUser(UserDao userDao);

    @Delete("DELETE FROM user WHERE u_id = #{id}")
    int deleteUserById(int id);

    @Update("UPDATE user SET u_skill = #{skill}, u_name = #{name},u_power = #{power}, u_type = #{type} WHERE u_id = #{id}")
